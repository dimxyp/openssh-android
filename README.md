...
- **Real interactive terminal emulator** (`terminal/TerminalEmulator.kt`, `ui/terminal/TerminalGrid.kt`): a from-scratch, original VT100/ANSI-subset parser and in-memory screen buffer (rows/cols grid of styled characters) - not derived from JuiceSSH, Termux, or any other terminal emulator. It supports printable characters, `\r`/`\n`/backspace/tab, cursor positioning (`ESC[<row>;<col>H`), relative cursor movement (`ESC[A/B/C/D`), erase in line/display (`ESC[K`, `ESC[J` and their `0`/`1`/`2` variants), and basic SGR attributes (`ESC[...m` bold + the 8 standard and 8 bright foreground/background colors; the extended `38;5;n`/`38;2;r;g;b` 256-color/truecolor forms are consumed and approximated rather than mis-parsed as separate attributes). Typing is live: every keystroke from a hidden input capture is streamed straight to the shell channel as it's typed (no "Send" button), and `SshSessionHolder` batches/throttles screen-buffer updates (~20 fps) so fast output (e.g. `top`) doesn't recompose the UI per byte.

  **Scrollback:** lines that scroll off the top of the screen are not discarded - they are kept in a bounded scrollback buffer (2000 lines by default, oldest dropped beyond that) and published as part of the immutable `TerminalSnapshot`, so the UI can render session history above the live screen.

  **Auto-sizing:** the grid is not a fixed 30x100 any more. The terminal UI measures the area actually available and the monospace glyph box (`BoxWithConstraints` + `rememberTextMeasurer`), derives `cols = width / charWidth` and `rows = height / lineHeight`, and pushes that size down through `AndroSshViewModel.onTerminalSizeChanged` to `SshSessionHolder.resizeTerminal`, which resizes the screen buffer *and* sends an SSH `window-change` to the server. The shell is therefore always started with an explicit `xterm-256color` PTY of the real dimensions (`stty size` reports what you see), and re-measures on rotation, split-screen and whenever the soft keyboard opens or closes - which is what makes readline line editing and full-screen programs like `vim`/`top` render correctly. Shrinking the screen keeps the newest lines and moves the oldest into scrollback.

  Known limitations: no support for alternate-screen mode, 256-color/truecolor SGR codes, or bracketed paste.
...
- Full xterm-compatible terminal emulation (256-color/truecolor, alternate screen, bracketed paste - see the terminal emulator's known limitations above).
- Port forwarding.

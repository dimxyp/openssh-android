...
- **Full-screen terminal UI**: once connected, the "AndroSSH" title and the outer padding are hidden so the terminal grid and the extra-keys bar use nearly the whole screen (system bar insets are still respected). The connected `user@host` is shown as a small translucent overlay in the top-right corner, together with a compact disconnect icon button (there is no full-width "Disconnect" button any more). Tapping the terminal area always re-opens the on-screen keyboard, even if it was dismissed with the IME's own back button.
- **Stay connected in the background** (`ssh/SshForegroundService.kt`): while a shell session is open, the app runs a foreground service showing a low-priority, ongoing "Connected to user@host" notification. This keeps the app process - and with it the SSH socket and the output-reading coroutine - alive when the app is minimised. The service is only a lifecycle anchor: the session itself still lives in `AndroSshViewModel`, so a process kill (e.g. swiping the task away) still drops the connection. The notification and service are removed as soon as you disconnect or go back to the connection list, and reconnecting reuses the same single notification. This requires the `FOREGROUND_SERVICE`, `FOREGROUND_SERVICE_DATA_SYNC` and `POST_NOTIFICATIONS` permissions and a `<service android:name=".ssh.SshForegroundService" android:foregroundServiceType="dataSync" />` declaration in `app/src/main/AndroidManifest.xml`.
